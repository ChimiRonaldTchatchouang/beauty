...
// Mise à jour des préférences de l'utilisateur courant (langue, notifications)
// et, pour un patient, de son profil peau.
export async function PATCH(req: Request) {
  ...
  if (typeof body.notifyRoutine === "boolean") userUpdate.notifyRoutine = body.notifyRoutine;
  ...
  if (typeof body.currentRoutine === "string") profileUpdate.currentRoutine = body.currentRoutine;
  if (Object.keys(profileUpdate).length > 0) {
    profileUpdate.updatedAt = new Date();
    await db
      .insert(skinProfiles)
      .values({ userId: session.userId, ...profileUpdate })
      .onConflictDoUpdate({ target: skinProfiles.userId, set: profileUpdate });
  }
