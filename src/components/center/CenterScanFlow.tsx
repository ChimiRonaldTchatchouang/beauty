...
function Flow({ patients, preselected }: { patients: Patient[]; preselected: string | null }) {
  ...
  function tipFor(q: QualityResult) {
    ...
    if (q.reason === "no_face") return "Centrez le visage du patient";
    ...
    while (attempt < 3) {
      try {
        const res = await fetch("/api/center/scan", {
          ...
          body: JSON.stringify({ patientId, image, thumbnail, quality }),
        });
        if (res.status === 402) {
        ...
        attempt++;
        if (attempt >= 3) {
          setError("Échec de l'analyse. Vérifiez la connexion et réessayez.");
          setPhase("error");
          return;
        }
      ...
      <div className="mx-auto max-w-md">
        ...
        {patients.length === 0 ? (
          <div className="card text-center">
            ...
            <Link href="/center/patients" className="btn-primary mt-4">
              Ajouter un patient
            </Link>
          </div>
        ) : (
  ...
  if (phase === "analyzing") {
    return (
      <div className="flex min-h-[60vh] flex-col items-center justify-center text-center">
        ...
        <p className="mt-2 text-ink-soft">Notre IA examine la peau, quelques secondes…</p>
      </div>
    );
  }
  ...
  if (phase === "results" && analysis) {
    return (
      <div>
        ...
        <ResultsView analysis={analysis} image={captured} showRoutineCta={false} />
        <div className="mt-6">
  ...
  return (
    <div className="mx-auto max-w-md">
      ...
      <p className="mb-4 text-sm text-ink-soft">Patient : {patient?.name ?? patient?.email}</p>
      ...
      <div className="relative aspect-[3/4] w-full overflow-hidden rounded-3xl bg-ink">
        ...
        {cameraReady && tip && (
          <div className="absolute inset-x-0 top-4 flex justify-center">
            <span className="rounded-full bg-black/55 px-4 py-1.5 text-sm font-medium text-white">{tip}</span>
          </div>
        )}
        {!cameraReady && (
      ...
      <div className="mt-5 flex items-center justify-center gap-4">
        <button onClick={() => fileRef.current?.click()} className="grid h-14 w-14 place-items-center rounded-full border border-sand-200 bg-white text-ink-soft" aria-label="Importer">
          <UploadIcon width={24} height={24} />
        </button>
        <button onClick={capture} disabled={!cameraReady} className="grid h-20 w-20 place-items-center rounded-full bg-brand-500 text-white shadow-soft ring-4 ring-brand-100 transition active:scale-95 disabled:opacity-40" aria-label="Capturer">
