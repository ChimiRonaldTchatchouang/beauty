...
export async function registerConsoleRoutes(app: FastifyInstance, repo: Repository): Promise<void> {
  ...
  app.get('/api/console/dashboard', async () => ({
    ...
    topQueries: await repo.topKbQueries(10),
  }));

  app.get('/api/console/calls', async () => ({ calls: await repo.listCalls(200) }));

  app.get('/api/console/calls/:id', async (req) => {
    const { id } = req.params as { id: string };
    const call = await repo.getCall(id);
    if (!call) return { call: null };
    return {
      call,
      turns: await repo.listTurns(id),
      tools: await repo.listToolCalls(id),
      sms: await repo.listSmsByCall(id),
    };
  });

  app.get('/api/console/kb', async () => ({ fiches: await repo.kbAll() }));
  ...
  app.patch('/api/console/kb/:id/verified', async (req) => {
    ...
    await repo.kbSetVerified(id, verified, new Date().toISOString().slice(0, 10));
    return { ok: true };
  });

  app.delete('/api/console/kb/:id', async (req) => {
    const { id } = req.params as { id: string };
    await repo.kbDelete(id);
    return { ok: true };
  });

  app.get('/api/console/tickets', async () => ({ tickets: await repo.listTickets(200) }));
